    ...
    public partial class Http : IHttp, IHttpFactory
    {
...
#else
                    if (header.Name.ToLower() == "referer")
                    {
        ...
        private string EncodeParameters()
        {
            ...
            foreach (HttpParameter p in this.Parameters)
            {
                ...
                {
                    querystring.AppendFormat("{0}={1}", System.Web.HttpUtility.UrlEncode(p.Name,encoding) , System.Web.HttpUtility.UrlEncode(p.Value, encoding));
                }
            }
